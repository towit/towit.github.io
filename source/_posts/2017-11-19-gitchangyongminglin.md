...
1. 修订整合命名：git status && git add . && git commit -m "Travis CI" && git push origin source
2. 新增整合命令：git init && git remote add origin git@github.com:towit/towit.github.io.git && git checkout --orphan source && git add . && git commit -m "Travis CI" && git push origin source
```
```
git init
